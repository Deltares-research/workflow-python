...
from workflowpy.utils.cwl_utils import map_cwl_types, wildcard_inputs_nested
    ...
    def _set_input(self) -> Dict[str, str]:
        ...
        for key, val in refs.items():
            ...
            if isinstance(ref.value, filedirpath):
                ...
                # This is to ensure the proxy file and the dir it represents have
                # the correct parentage
                inputs[key]["valueFrom"] = f"$(inputs.{key}_dir.path)/$(self.basename)"
            # ref to config does not maintain folderpath typing
            # (reverts it instead to Path)
    ...
    def _set_output(self):
        ...
        for step in self.steps:
            for id, info in step.output.items():
                ...
                # Make sure outputs produced by subworkflows are labeled
                # as array outputsThis can give output types with [][],
                # corrected for in jinja template
    ...
    def _set_input_scatter(self) -> List[str]:
        ...
        for key, info in ins.items():
            if "_wc" in key:
                ...
                # Wildcards that are only scattered over in a
                # subworkflow should be array inputs

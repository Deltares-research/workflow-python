...
class Method(ABC):
    ...
    It should have a name, input, output and params,
    and implement a _run and __init__ method.
    """
    ...
    def __init__(self) -> None:
        ...
        # NOTE: wildcards on outputs should be defined on file parent,
        # not the file name itself!
    ...
    def to_kwargs(
        ...
        **kwargs,
        ...
        """Return flattened keyword-arguments."""
        kwargs = dict(
    ...
    def from_kwargs(cls, method_name: Optional[str] = None, **kwargs) -> "Method":
        ...
        From the method `name` and its initialization arguments.
        """
    ...
    ## TESTING METHODS
    # (we keep these here such that external implementations can use them)
    ...
    def _test_unique_keys(self) -> None:
        ...
        inputs = list(self.input.__class__.model_fields.keys())
        outputs = list(self.output.__class__.model_fields.keys())
        params = list(self.params.__class__.model_fields.keys())
        ...
        for key, value in self.input.model_dump().items():
            if isinstance(value, Path):
                if has_wildcards(value):
                    raise ValueError(
                        f"The method {self.name} has unresolved wildcards in \
input paths. Please create a rule to resolve these wildcards and run the rule."
                    )

...
from workflowpy.wildcards import wildcard_product
...
def map_cwl_types(input: Any) -> Dict[str, str]:
    ...
    -------
    Dict[str,str]
        Dict containing variable CWL type (used in both rule and workflow files)
        and its value (used to generate config file)
    ...
    TypeError
        Raised when a variable type could not parsed to CWL equivalent.
        Parsing to CWL string is tried as default.
    """
    out = {}
    match input:
        case bool():

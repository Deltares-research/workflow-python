...
class Rules:
    ...
    def _detect_dependencies_rule(self, rule: Rule) -> list[str]:
        """Find all dependencies of rule_id.

        Done by matching input values to output values of prev rules.
        This method updates dependency_map and returns list with
        rule_id of dependencies.
        """
    ...
    def _get_new_rule_index(self, rule: Rule) -> int:
        ...
        if len(dependencies) > 0:
            ...
            # If there is already a rule in that position,
            # break tie based on loop_depth with highest loop depth last
            if ind != len(self.names) and rule._loop_depth > self[ind]._loop_depth:
                ind += 1
        # If rule input does not depend on others,
        # put at beginning after other rules with no input dependencies.
        else:
            ind = len(
                [
                    rule_id
    ...
    # Sort repeat wildcards here vs in Rule class,
    # because we need access to previous rule and its wildcard order.

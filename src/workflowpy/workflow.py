...
class Workflow:
    ...
    def root(self, root: Path) -> None:
        """Set the root of the workflow.

        Also create the directory if it does not yet exist.
        """
    ...
    def get_ref(self, ref: str) -> Ref:
        ...
        To either previously set rule parameters or workflow config.
        """
        return Ref(ref, self)
    ...
    def to_snakemake(
        ...
        template_env = Environment(
            loader=PackageLoader("workflowpy"),
            trim_blocks=True,
            lstrip_blocks=True,
        )
    ...
    def to_cwl(
        ...
        cwlfile : Path
            ...
            CWL files for individual methods will be created
            in the subfolder <cwlfile>/cwl.
        dryrun : bool, optional
            Run the workflow in dryrun mode, by default False
        """
        if cwlfile is None:
        ...
        template_env = Environment(
            loader=PackageLoader("workflowpy"), trim_blocks=True, lstrip_blocks=True
        )
        template_workflow = template_env.get_template("workflow.cwl.jinja")
        template_rule = template_env.get_template("rule.cwl.jinja")

        # Make a copy of workflow so any changes CWL parser
        # will make to config, refs do not alter original
    ...
    def plot_rulegraph(
        ...
        The rulegraph is a directed graph where the nodes are rules
        and the edges are dependencies.

        The nodes are colored based on the type of rule:
        Expand rules are colored blue, reduce rules are colored green,
        and rules with repeat wildcards are colored purple.
        Result rules are colored red.
        ...
        plot_rule_attrs : bool, optional
            Plot the rule attributes (n runs and wildcards) in the node,
            by default True.

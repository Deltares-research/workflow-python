...
class Rule:
    ...
    def wildcard_fields(self) -> Dict[str, List]:
        ...
        Per wildcard it contains all input, output and
        params field names which have the wildcard.
        """
    ...
    def _validate_wildcards(self) -> None:
        ...
        if isinstance(self.method, ExpandMethod) and not self.wildcards["expand"]:
            msg = f"ExpandMethod {self.method.name} requires a new expand \
wildcard on output (Rule {self.rule_id})."
        elif isinstance(self.method, ReduceMethod) and not self.wildcards["reduce"]:
            msg = f"ReduceMethod {self.method.name} requires a reduce \
wildcard on input only (Rule {self.rule_id})."
        elif self.wildcards["expand"] and not isinstance(self.method, ExpandMethod):
            wcs = self.wildcards["expand"]
            msg = f"Wildcard(s) {wcs} missing on input or method {self.method.name} \
should be an ExpandMethod (Rule {self.rule_id})."
        elif self.wildcards["reduce"] and not isinstance(self.method, ReduceMethod):
            wcs = self.wildcards["reduce"]
            msg = f"Wildcard(s) {wcs} missing on output or method {self.method.name} \
should be a ReduceMethod (Rule {self.rule_id})."
        if msg:
    ...
    def _wildcard_product(self) -> List[Dict[str, str]]:
        ...
        # drop None from list of values; this occurs when
        # the workflow is not fully initialized yet
    ...
    def _output_path_refs(self) -> Dict[str, str]:
        ...
        -------
        Dict[str, str]
            Dictionary containing the output path as the key
            and the reference as the value.
        """
    ...
    def _create_references_for_method_inputs(self):
        """Create references for method inputs.

        Based on output paths of previous rules.
        """
        ...
        for key, value in self.method.output:
            ...
            if value in output_path_refs:
                ...
                raise ValueError(
                    f"All output file paths must be unique. \
{self.rule_id}.output.{key} ({value}) is already an output of {duplicate_field}"
                )
                ...
                for key, value in inout_dict.items():
                    ...
                    if name in ["input", "output"]:
                        ...
                        if not isinstance(value[0], Path):
                            continue
                        # Removes duplicates
                        # Using set() does not preserve insertion order,
                        # this does and also filters uniques
                        for val in value:
